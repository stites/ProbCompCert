...
Lemma eval_lvalue_correct:
  forall e le m a b ofs target ta bf
  (TRE: transf_etarget_expr (prog_target prog) a = OK ta),
    CStanSemanticsTarget.eval_lvalue ge e le m target a b ofs bf ->
    CStanSemanticsBackend.eval_lvalue tge e le m ta b ofs bf.
Proof.
  intros e le m a.
...
Proof.
  ...
  - (* set *)
    ...
    + (* in other *)
      monadInv TRS; monadInv EQ. (* simpl. monadInv EQ0. *)
      econstructor.
      split. eapply plus_one.
      destruct (id =? (prog_target prog))%positive eqn:TEQ; monadInv EQ2.
      monadInv EQ0.
      unfold stepf.
      econstructor.
      eapply eval_expr_correct; eauto.
